...
public class SudokuPanel extends JPanel {
    ...
    private static Sudoku9x9 modelSudoku;
    ...
    private void resetColor() {
        ...
        currentFocus.setX(NOT_FOCUS);
        currentFocus.setY(NOT_FOCUS);
    }
    ...
    private void restoreColor(Coordinates c) {
        ...
        int i = (x / SudokuConfig.SMALL_BOX_SIZE) * SudokuConfig.SMALL_BOX_SIZE;
        int k = (y / SudokuConfig.SMALL_BOX_SIZE) * SudokuConfig.SMALL_BOX_SIZE;
        final int n = i + SudokuConfig.SMALL_BOX_SIZE;
        final int m = k + SudokuConfig.SMALL_BOX_SIZE;
        for (; i < n; ++i)
    ...
    public boolean setTextToMatrix(String num) {
        if (currentFocus.getX() == NOT_FOCUS || currentFocus.getY() == NOT_FOCUS)
            return false;
        if (modelSudoku.isCorrectNumber(currentFocus, num)) {
            matrixKey[currentFocus.getX()][currentFocus.getY()].setText(num);
            matrixKey[currentFocus.getX()][currentFocus.getY()].setForeground(ColorConfig.COLOR_KEY_CORRECT);
    ...
    public void startSudoku() {
        ...
        int level = SudokuConfig.LEVEL_EASY;
        gstate = new GameState(level, max);
        for (i = 0; i < level; ++i) {
            x = A.get(i) / 9;
            y = A.get(i) % 9;
        ...
        for (i = 0; i < TYPE_SUDOKU_SIZE; ++i) {
            for (j = 0; j < TYPE_SUDOKU_SIZE; ++j) {
                ...
                if (current.getText().isEmpty()) {
                    ...
                    current.setForeground(ColorConfig.COLOR_KEY_CORRECT);
                }
            }
        }
    }
